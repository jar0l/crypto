...
{
	public sealed class MarsCipher : IBlockCipher
	{
		#region S-Box
		private static readonly uint[] _sBox = new uint[]
			{
		...
		#region Sub-key fixing procedure
		private static uint FixSubKey( uint key, uint prev )
		{
			uint i = key & 0x3;
			key |= 0x3;
			...
			m1 &= ((~key) ^ (key << 1)) & ((~key) ^ (key >> 1)) & 0x7FFFFFFC;

			key ^= Helper.RotateLeft( _sBox[265 + i], (int) prev ) & m1;
		...
		#region Data mixing
		private static void ApplyForwardMixing( ref uint x0, ref uint x1, ref uint x2, ref uint x3 )
		{
			x1 ^= _sBox[x0 & 0xFF];
			x1 += _sBox[((x0 >> 8) & 0xFF) | 0x100];
			x2 += _sBox[(x0 >> 16) & 0xFF];
			x0 = Helper.RotateRight( x0, 24 );
			x3 ^= _sBox[(x0 & 0xFF) | 0x100];
		}

		private static void ApplyBackwardMixing( ref uint x0, ref uint x1, ref uint x2, ref uint x3 )
		{
			x1 ^= _sBox[(x0 & 0xFF) | 0x100];
			x2 -= _sBox[(x0 >> 24) & 0xFF];
			x3 -= _sBox[((x0 >> 16) & 0xFF) | 0x100];
			x0 = Helper.RotateLeft( x0, 24 );
			x3 ^= _sBox[x0 & 0xFF];
		}
		#endregion

		#region Core rounds
		private static void PerformCoreRound( ref uint x0, ref uint x1, ref uint x2, ref uint x3, uint k0, uint k1 )
		{
			uint m = x0 + k0;
			x0 = Helper.RotateLeft( x0, 13 );
			uint r = x0 * k1;
			uint l = _sBox[m & 0x1FF];
			r = Helper.RotateLeft( r, 5 );
			x2 += Helper.RotateLeft( m, (int) r );
			l ^= r;
			r = Helper.RotateLeft( r, 5 );
			l ^= r;
			x3 ^= r;
			x1 += Helper.RotateLeft( l, (int) r );
		}

		private static void PerformInverseCoreRound( ref uint x0, ref uint x1, ref uint x2, ref uint x3, uint k1, uint k0 )
		{
			uint r = x0 * k1;
		...
		public MarsCipher( byte[] key, bool encrypt )
		{
			if ( key == null )
			...
			// Phase 1: copy the key material followed by the number of words to a temporary array 't'
			uint[] t = new uint[15];
			for ( int i = 0; ; )
			{
				t[i] = Helper.ToLEUInt32( key, i * 4 );
			...
			// Phase 2: compute the words of the expanded keys with 4 cycles
			for ( int j = 0; j < 4; j++ )
			{
				// Linear transformation
				for ( int i = 0; i < t.Length; i++ )
					t[i] ^= Helper.RotateLeft( t[(i + 8) % 15] ^ t[(i + 13) % 15], 3 ) ^ (uint)( 4 * i + j );

				// Four stirring rounds
				for ( int k = 0; k < 4; k++ )
				{
					for ( int i = 0; i < t.Length; i++ )
						t[i] = Helper.RotateLeft( t[i] + _sBox[ t[(i + 14) % 15] & 0x1FF ], 9 );
				}

				// Take 10 words of t as the next ten expanded keys
				for ( int i = 0; i < 10; i++ )
					_keys[10 * j + i] = t[(4 * i) % 15];
			}


			// Phase 3: fix the multiplication keys
			for ( int i = 5; i < 36; i += 2 )
				_keys[i] = FixSubKey( _keys[i], _keys[i - 1] );
		}

		private void Encrypt( ref uint x0, ref uint x1, ref uint x2, ref uint x3 )
		{
			// Add sub-keys to data
			x0 += _keys[0];
			x1 += _keys[1];
			x2 += _keys[2];
			x3 += _keys[3];

			// 8 rounds of forward mixing
			for ( int i = 0; i < 2; i++ )
			{
				ApplyForwardMixing( ref x0, ref x1, ref x2, ref x3 ); x0 += x3;
				ApplyForwardMixing( ref x1, ref x2, ref x3, ref x0 ); x1 += x2;
				ApplyForwardMixing( ref x2, ref x3, ref x0, ref x1 );
				ApplyForwardMixing( ref x3, ref x0, ref x1, ref x2 );
			}

			// 8 forward core rounds
			for ( int i = 4; i < 20; )
			{
				PerformCoreRound( ref x0, ref x1, ref x2, ref x3, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x1, ref x2, ref x3, ref x0, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x2, ref x3, ref x0, ref x1, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x3, ref x0, ref x1, ref x2, _keys[i++], _keys[i++] );
			}

			// 8 backward core rounds
			for ( int i = 20; i < 36; )
			{
				PerformCoreRound( ref x0, ref x3, ref x2, ref x1, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x1, ref x0, ref x3, ref x2, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x2, ref x1, ref x0, ref x3, _keys[i++], _keys[i++] );
				PerformCoreRound( ref x3, ref x2, ref x1, ref x0, _keys[i++], _keys[i++] );
			}

			// 8 rounds of backward mixing
			for ( int i = 0; i < 2; i++ )
			{
				ApplyBackwardMixing( ref x0, ref x1, ref x2, ref x3 );
				ApplyBackwardMixing( ref x1, ref x2, ref x3, ref x0 ); x2 -= x1;
				ApplyBackwardMixing( ref x2, ref x3, ref x0, ref x1 ); x3 -= x0;
				ApplyBackwardMixing( ref x3, ref x0, ref x1, ref x2 );
			}

			// Subtract keys to data
			x0 -= _keys[36];
			x1 -= _keys[37];
			x2 -= _keys[38];
			x3 -= _keys[39];
		...
		private void Decrypt( ref uint x0, ref uint x1, ref uint x2, ref uint x3 )
		{
			...
			for ( int i = 0; i < 2; i++ )
			{
				ApplyForwardMixing( ref x3, ref x2, ref x1, ref x0 ); x3 += x0;
				ApplyForwardMixing( ref x2, ref x1, ref x0, ref x3 ); x2 += x1;
				ApplyForwardMixing( ref x1, ref x0, ref x3, ref x2 );
				ApplyForwardMixing( ref x0, ref x3, ref x2, ref x1 );
			}

			// 8 forward inverse core rounds
			for ( int i = 35; i > 19; )
			{
				PerformInverseCoreRound( ref x3, ref x2, ref x1, ref x0, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x2, ref x1, ref x0, ref x3, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x1, ref x0, ref x3, ref x2, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x0, ref x3, ref x2, ref x1, _keys[i--], _keys[i--] );
			}

			// 8 backward inverse core rounds
			for ( int i = 19; i > 3; )
			{
				PerformInverseCoreRound( ref x3, ref x0, ref x1, ref x2, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x2, ref x3, ref x0, ref x1, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x1, ref x2, ref x3, ref x0, _keys[i--], _keys[i--] );
				PerformInverseCoreRound( ref x0, ref x1, ref x2, ref x3, _keys[i--], _keys[i--] );
			}

			// 8 rounds of backward mixing
			for ( int i = 0; i < 2; i++ )
			{
				ApplyBackwardMixing( ref x3, ref x2, ref x1, ref x0 );
				ApplyBackwardMixing( ref x2, ref x1, ref x0, ref x3 ); x1 -= x2;
				ApplyBackwardMixing( ref x1, ref x0, ref x3, ref x2 ); x0 -= x3;
				ApplyBackwardMixing( ref x0, ref x3, ref x2, ref x1 );
			}
